...
function simple_history_get_items_array($args = "") {
    ...
    // retrieve blog table - ey
    $bloglist = array();
    $sql = "SELECT * FROM wp_blogs WHERE 1";
    $rows = $wpdb->get_results($sql);
    foreach ($rows as $the_row) {
        $bloglist[$the_row->blog_id] = /*$the_row->domain.*/$the_row->path;
    }
    
    // retrieve write panel names - ey
    $write_panels = array();
    $sql = "SELECT * FROM wp_mf_write_panels WHERE 1";
    $rows = $wpdb->get_results($sql);
    foreach($rows as $the_row) {
        $write_panels[$the_row->id] = $the_row->name;
    }
    
    //echo "bloglist=".print_r($bloglist,true);
    ...
    // get history from all sites - ey
    // todo: make this less hard coded

    $sql = "SELECT *,1 as blog_id,t2.meta_value as page_type_id FROM wp_simple_history as t1 ";
    $sql .= "LEFT JOIN wp_postmeta as t2 ON t2.post_id = t1.object_id AND t2.meta_key = '_mf_write_panel_id' ";
    $sql .= $where;
    $sql .= " UNION ";
    $sql .= "SELECT *,4 as blog_id,t2.meta_value as page_type_id FROM wp_4_simple_history as t1 ";
    $sql .= "LEFT JOIN wp_4_postmeta as t2 ON t2.post_id = t1.object_id AND t2.meta_key = '_mf_write_panel_id' ";
    $sql .= $where;
    $sql .= " UNION ";
    $sql .= "SELECT *,5 as blog_id,t2.meta_value as page_type_id FROM wp_5_simple_history as t1 ";
    $sql .= "LEFT JOIN wp_5_postmeta as t2 ON t2.post_id = t1.object_id AND t2.meta_key = '_mf_write_panel_id' ";
    $sql .= $where;
...
#print_r($rows);
    ...
    if ($rows) {
        ...
        // this loop combines multiposts into a single entry
        //echo "loop 1<br>";
        foreach ($rows as $one_row) {
            $one_row->id = $loopNum;
            
            // $one_row->id." ";
            //echo $one_row->date." ".$one_row->object_name . " " . $one_row->action . " " . $one_row->blog_id." ";
            
            // add blog_name and page_type - ey
            $one_row->blog_name = $bloglist[$one_row->blog_id];             // ey
            $one_row->page_type = $write_panels[$one_row->page_type_id];    // ey
            $one_row->time = strtotime($one_row->date);                     // ey
            $do_add = TRUE;
            //$t = $one_row->time; echo strftime("%Y-%m-%d %H:%M:%S",$t)." $t ";
            ...
            if (//0 && // occasion feature disabled - ey
                    $prev_row 
                    && ($one_row->action=="updated" || $one_row->action=="created" || $one_row->action=="deleted")
                    && $one_row->blog_id != $prev_row->blog_id
                    && $one_row->object_type == $prev_row->object_type
                    && $one_row->object_subtype == $prev_row->object_subtype 
                    && $one_row->user_id == $prev_row->user_id 
                    && (
                      (!empty($one_row->object_id) 
                      && !empty($prev_row->object_id)) 
                      && ($one_row->object_id == $prev_row->object_id) 
                      || ($one_row->object_name == $prev_row->object_name)
                    )
            ) {
                // if the time is only off by 10 seconds and the blog ID is not the same - ey
                if ( $prev_row && $one_row->blog_id != $prev_row->blog_id
                    && ($prev_row->time - $one_row->time) < 15 ) {
                    $rows2[$prev_row->id]->multipost[] = $one_row;
                    $do_add = FALSE;
                    //echo "combine prev_row=".$prev_row->id.' ';
                    
                    // if one of the multi posts are "created" then set the main one to "created"
                    if ($prev_row->action=="updated" && $one_row->action=="created")
                        $rows2[$prev_row->id]->action = "created";
                }
            }
            if ($do_add) $real_loop_num++;

            // new event, not as previous one								
            if ($do_add) {
                $rows2[$one_row->id] = $one_row;
                $rows2[$one_row->id]->multipost = array(); // ey
                $loopNum++;
                $prev_row = $one_row;
            }
            //echo "<br>";
...
#print_r($rows2);
        ...
        foreach ($rows2 as $one_row) {
            ...
            $one_row->id = $loopNum;                                        // ey
            //echo $one_row->id." ";
            //echo $one_row->date." ".$one_row->object_name . " " . $one_row->action . " " . $one_row->blog_id." ";
            //if (count($one_row->multipost)) echo "multipost ";
            ...
            if (//0 && // occasion feature disabled - ey
                    ...
                    //echo " occasion prev_row=".$prev_row->id." ";
            } else {
    ...
    if (is_numeric($args["items"]) && $args["items"] > 0) {
        ...
        #echo "offset: $offset count: " . $args["items"];
	...
	if ($arr_events) {
		...
		foreach ($arr_events as $one_row) {
			...
			$occasions = $one_row->occasions;
                        ...
                        $blog_id = $one_row->blog_id;       // ey
                        $blog_name = $one_row->blog_name;   // ey
                        $page_type = $one_row->page_type;   // ey
                        $tag = $one_row->tag;               // ey

                        // setup multipost buttons - ey
                        $mp_blogs = array(1=>"", 4=>"",5=>"");
                        $btn_name = "main";
                        if ($one_row->blog_id != 1) $btn_name = str_replace("/","",$one_row->blog_name);
                        $mp_blogs[$one_row->blog_id] = $btn_name;
                        
                        switch_to_blog($blog_id);           // ey
			...
			if ("post" == $object_type_lcase) {
				...
				$post_out .= esc_html__(ucfirst($object_subtype));
                                if (count($page_type)) $post_out .= " - <span style='color: #a00'>$page_type</span> ";   // ey 
				...
				if (null == $post) {
					...
					$edit_link = get_edit_post_link($object_id, 'display');
                                        $mp_onclick = "onclick='jQuery(\".mpc\").hide();jQuery(\"#mp".$one_row->id."\").show()'";
                                        if (count($one_row->multipost)==0)
                                            $post_out .= " <a href='$edit_link' >";
					else
                                            $post_out .= " <a href='#' class='mpa' $mp_onclick>";
					$post_out .= "<span class='simple-history-title'>{$title}</span>";
					$post_out .= "</a> ";
                                        if (count($one_row->multipost)) {
                                            $edit_link = get_edit_post_link($one_row->object_id, 'display');
                                            $post_out .= " <span class='mptag'><a class='mpa' href='#' $mp_onclick>multipost</a><span class='mpc' id='mp".$one_row->id."' style='display:none'>";
                                            $post_out .= "<a class='mpbtn' target='_blank' href='".$edit_link."'>$btn_name</a> ";

                                            // 2nd blog found
                                            if (isset($one_row->multipost[0])) {
                                                $tmp_row = $one_row->multipost[0];
                                                $btn_name = "main";
                                                if ($tmp_row->blog_id != 1) $btn_name = str_replace("/","",$tmp_row->blog_name);
                                                switch_to_blog($tmp_row->blog_id);           // ey
                                                $edit_link = get_edit_post_link($tmp_row->object_id, 'display');
                                                restore_current_blog();
                                                $post_out .= "<a class='mpbtn' target='_blank' href='".$edit_link."'>$btn_name</a> ";
                                                $mp_blogs[$tmp_row->blog_id] = $btn_name;
                                            }
                                            // 3rd blog found
                                            if (isset($one_row->multipost[1])) {
                                                $tmp_row = $one_row->multipost[1];
                                                $btn_name = "main";
                                                if ($tmp_row->blog_id != 1) $btn_name = str_replace("/","",$tmp_row->blog_name);
                                                switch_to_blog($tmp_row->blog_id);           // ey
                                                $edit_link = get_edit_post_link($tmp_row->object_id, 'display');
				...
				$output .= ucwords(esc_html__($object_type, "simple-history")) . " " . ucwords(esc_html__($object_subtype, "simple-history")) . " <span class='simple-history-title'>\"" . esc_html($object_name) . "\"</span> " . esc_html($unknown_action);
                        ...
                        // display blog reference - ey
                        $output .= "<div style='float:right' id=''>";
                        $output .= "<table cellpadding='0' cellspacing='1' border='0'><tr>";
                        foreach($mp_blogs as $mp_blog_id => $one_blog) {
                            $mpclass = "mpclass";
                            if ($one_blog != "") $mpclass .= $mp_blog_id;
                            $output .= "<td align='center' class='$mpclass' width='30'>$one_blog</td>";
                        }
                        $output .= "</tr></table>";
                        $output .= "</div>";
			...
			$now = strtotime(current_time("mysql"));
                        $diff_str = $one_row->date.sprintf( __(' (<span class="when">%1$s ago</span>) by %2$s', "simple-history"), human_time_diff(strtotime($one_row->date), $now), $who );
			$output .= $diff_str;
			...
			if ($num_occasions > 0) {
				...
				foreach ($occasions as $one_occasion) {
					...
					$output .= sprintf( __('%s ago (%s at %s)', "simple-history"), human_time_diff(strtotime($one_occasion->date), $now), $date_i18n_date, $date_i18n_time );
                                        //$output .= " ".$one_occasion->action." ".$one_occasion->object_name." ".$one_occasion->blog_name; // ey
					$output .= "</li>";
				}
				$output .= "</ul>";
				$output .= "</div>";
        ...
        $output .= "<script> jQuery('.mpa').click(function(e) {e.preventDefault();}); </script>";    // ey
	return $output;
}
